...
if (type == 'image') {
alpha.sendMessage(from, { caption: text, image: options ? options : fs.readFileSync(thumbnail), buttons: buttons, headerType: 'IMAGEN', mentions: men }, {quoted: m})
} else if (type == 'video') {
if (options === undefined || options === null) return reply('método ilegal, bot propietario de chat')
alpha.sendMessage(from, { caption: text, video: options, buttons: buttons, headerType: 'VIDEO', mentions: men }, {quoted: m})
} else if (type == 'location') {
alpha.sendMessage(from, { caption: text, location: { jpegThumbnail: options ? options : fs.readFileSync(thumbnail) }, buttons: buttons, headerType: 'UBICACION', mentions: men })
} else if (type == 'text') {
...
alpha.relayMessage(anu, buatpesan.message, { messageId: buatpesan.key.id })
        ...
        switch(command) {
            ...
            case 'linkgroup': case 'linkgrup': case 'linkgc': {
                ...
                alpha.sendText(m.chat, `Link Grupo : *${groupMetadata.subject}*\nhttps://chat.whatsapp.com/${response}`, m, { detectLink: true })
            }
            break
	    ...
	    case 'toimage': case 'toimg': {
                ...
                if (!/webp/.test(mime)) throw `Pegatina de respuesta con el comando *${prefix + command}*`
                m.reply(mess.wait)
            ...
            case 'hentai': {
                ...
                anu = await hentai()
                result = anu[Math.floor(Math.random(), anu.length)]
                alpha.sendMessage(m.chat, { video: { url: result.video_1 }, caption: `⭔ Titulo : ${result.title}\n⭔ Categoría : ${result.category}\n⭔ Mimetipo : ${result.type}\n⭔ Views : ${result.views_count}\n⭔ Acciones : ${result.share_count}\n⭔ Fuente : ${result.link}\n⭔ Media Url : ${result.video_1}` }, { quoted: m })
            }
            break
            ...
            case 'public': {
                if (!m.key.fromMe && !isCreator) throw mess.owner
                alpha.public = true
                m.reply('Cambiar correctamente al modo público')
            }
            break
            case 'self': {
                if (!m.key.fromMe && !isCreator) throw mess.owner
                alpha.public = false
                m.reply('Cambiar con éxito al modo Self')
            }
            break
            case 'ping': case 'botstatus': case 'statusbot': {
                ...
                respon = `
Velocidad de respuesta ${latensi.toFixed(4)} _Segundo_ \n ${oldd - neww} _Milisegundos_\n\nRuntime : ${runtime(process.uptime())}

💻 Info Server
RAM: ${formatp(os.totalmem() - os.freemem())} / ${formatp(os.totalmem())}

_NodeJS Memoria Usaage_
...
try {
if (!quoted) return m.reply(`Reply Image/Video con comando ${prefix + command}\n\n*Nota*: _Duración Sticker Video/Gif 1-9 segundos_`)
if (!/image/.test(mime) && !/video/.test(mime)) return m.reply(`Reply Image/Video Con comando ${prefix + command}\n\n*Note*: _Duracion Sticker Video/Gif 1-9 Segundo_`)
let media = await alpha.downloadAndSaveMediaMessage(quoted)
await ffmpeg(`${media}`)
.input(media)
...
m.reply(`Reply Image/Video con comando ${prefix + command}\n\n*Nota*: _Duracion Sticker Video/Gif 1-9 segundos_`)
}
...
case 'mp4': case 'ytmp4':
                ...
                xfar.Youtube(text).then(async (data) => {
                    ...
                    txt += `*🎞️ Tipo :* ${data.medias[1].extension}\n`
                    txt += `*💾 Tamaño :* ${data.medias[1].formattedSize}\n`
                    txt += `*📚 Url Fuente :* ${data.url}\n\n`
                    txt += `*Por favor, espere un minuto, está en proceso de envío...*`
            ...
            case 'setprofile': case 'setpp': {
                ...
                if (!quoted) throw 'Reply Image'
                if (/image/.test(mime)) throw `Responder a la imagen con el comando *${prefix + command}*`
            ...
            case 'facebook': case 'fb': case 'fbdl': case 'facebookdl':
                ...
                xfar.Facebook(args[1]).then(async data => {
                    ...
                    txt += `*📫 Titull :* ${data.title}\n`
                    txt += `*🎞️ Tipo :* ${data.medias[0].extension}\n`
                    txt += `*📟 Calidad :* ${data.medias[0].quality}\n`
                    txt += `*💾 Tamaño :* ${data.medias[0].formattedSize}\n`
                    txt += `*📚 Url :* ${data.url}`
            ...
            case 'mp3': case 'ytmp3':
                ...
                xfar.Youtube(q).then(async (data) => {
                    ...
                    txt += `*🎞️ Tipo :* ${data.medias[7].extension}\n`
                    txt += `*💾 Tamaño :* ${data.medias[7].formattedSize}\n`
                    txt += `*📚 Url Fuente :* ${data.url}\n\n`
                    txt += `*Por favor, espere un minuto, está en proceso de envío...*`
                    sendFileFromUrl(m.chat, data.thumbnail, txt, m)
